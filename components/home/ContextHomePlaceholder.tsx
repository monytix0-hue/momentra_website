...
type ContextHomePlaceholderProps = {
  variant: "personal" | "group" | "business";
  title: string;
};
...
export function ContextHomePlaceholder(props: ContextHomePlaceholderProps) {
  ...
  return <ContextHomePlaceholderLegacy variant={variant} title={title} />;
}

// Legacy implementation for personal and business — group routes to GroupHomePlaceholder above.
...
import { BUSINESS_CREATE_OPEN_EVENT } from "@/lib/businessShellEvents";
...
function TabPanel({ title, bottomPadding }: { title: string; bottomPadding: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm opacity-70">Placeholder — feature screens coming soon</p>
    </div>
  );
}
...
function ContextHomePlaceholderLegacy({
  variant,
  title,
  ...
  /** Always load business session on Business home — even when bootstrap says EMPTY. */
  ...
  const personalDataFetchUnblocked =
    sessionCacheHint || hasActiveMoment || createOptionsHydrating;
  const pulseEnabled =
    variant === "personal" &&
    (shouldLoadTabData(resolveScreen("personal", "pulse", bootstrap)) ||
      personalDataFetchUnblocked);
  ...
  useEffect(() => {
    if (variant !== "personal" || !sessionCacheHint || !bootstrap) return;
    // Personal layer stays mounted under Group/Business — only force-reload when shell is personal.
    if (getContextSnapshot().selectedContext !== "personal") return;
    if (
      contextStateFor("MY_MONEY") === "EMPTY" ||
      moduleStateFor("pulse") === "EMPTY" ||
      moduleStateFor("moments") === "EMPTY"
    ) {
      void loadBootstrap({ force: true });
    }
  }, [variant, sessionCacheHint, bootstrap]);
  ...
  function handleTabSelect(tab: BottomNavTabId) {
    ...
    if (variant === "business" && tab !== "add" && tab !== selectedTab) {
      bumpBusinessSessionGeneration();
    }
  ...
  // ACTIVE open: bootstrap only — zero create-options.
  useEffect(() => {
    if (!businessSessionEnabled) return;
    let cancelled = false;
    void (async () => {
      const sessionBootstrap = await ensureBusinessBootstrap(
        businessSessionRetryKey > 0,
      );
      if (cancelled || !sessionBootstrap) return;
      ...
      // Warm action catalog for selected moment only (not all surfaces).
  ...
  }, [businessSessionEnabled, businessSessionRetryKey]);
  ...
  async function refreshAfterBusinessManage(opts?: {
    ...
    await refreshBusinessSessionInventory(true);
    const snap = getBusinessSessionSnapshot();
  ...
  /** Defer until setup GET completes so open critical path prioritizes GET /setup. */
  function refreshBusinessSessionAfterSetupReady() {
    void refreshBusinessSessionInventory(true).then(() => {
      markBusinessSetupBootstrapDone();
    });
  }
  ...
  function openActiveMomentPulse(typeCode: PersonalMomentTypeCode) {
    ...
    void refreshMomentsAfterSetup();
    void refreshMemoryAfterSetup();
    void refreshLifeAfterSetup();
  }
  ...
  async function handleMomentActivated() {
    ...
    setSelectedMomentTypeCode(setupTypeCode);
    closeMomentSetup();
    setShowCreateOverlay(false);
    setPreviousTab(selectedTab);
    setSelectedTab("pulse");
    invalidatePersonalMomentsCache();
    invalidatePersonalPulseCache();
    invalidatePersonalMemoryCache();
  ...
  useEffect(() => {
    ...
    if (variant === "personal") {
      ...
      return () => window.removeEventListener(PERSONAL_CREATE_OPEN_EVENT, openCreate);
    }
    if (variant === "business") {
      window.addEventListener(BUSINESS_CREATE_OPEN_EVENT, openCreate);
      return () => window.removeEventListener(BUSINESS_CREATE_OPEN_EVENT, openCreate);
    }
    return undefined;
  }, [variant]);
  ...
  function renderPersonalContent() {
    switch (visibleTab) {
      ...
      default:
        return (
          <TabPanel
            title={`${title} · ${tabTitles[visibleTab]}`}
            bottomPadding={bottomPadding}
          />
        );
    }
  ...
  function renderBusinessContent() {
    ...
    // Active non–Team Ops verticals ship in Runs 9–10 — keep honest empty shell for now.
    return renderBusinessEmptyShell();
  }
      ...
      {variant === "business" && showBusinessSetup && businessSetupMomentId ? (
        <BusinessMomentSetup
          ...
          onActivated={() => {
            ...
            setBusinessSetupMomentId(null);
            setBusinessSetupSeed(null);
            // Personal-style: seed active surfaces in parallel with session bootstrap.
